#!/usr/bin/env python3
"""Validate the public HTML-first pptx package contract."""
...
REQUIRED_FILES = {
    ...
    "references/pptx-audit.md",
    "references/pptx-audit-rules.json",
}
EXPECTED_PYTHON = {
    "build_deck.py", "catalog.py", "cdp_validate.py", "doctor.py", "icon_registry.py",
    "html_urls.py", "media_assets.py", "outline.py", "package_manifest.py", "pptx_export.py", "pptx_audit.py",
    "preview_deck.py", "project.py", "slide_html.py",
    "state.py", "theme.py", "validate_skill.py", "workflow.py", "workflow_contract.py",
    "pptx_cli.py", "test_pptx_audit.py", "audit_skill.py",
}
...
def _validate_starter(path: Path, errors: list[str]) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="pptx-starter-") as directory:
...
def validation_errors() -> list[str]:
    ...
    wrapper = ROOT / "scripts" / "pptx"
    if wrapper.is_file() and not os.access(wrapper, os.X_OK):
        errors.append("scripts/pptx must be executable")
    ...
    if re.search(r"https?://|(?:[\"'(=]\s*)//[a-z0-9]", runtime_text, re.I):
    ...
    for token in (
        "text-body", "text-compact", "text-caption",
        "leading-display", "leading-body", "tracking-display",
    ):
    ...
    for selector in (".tag", ".hl"):
        if re.search(rf"(?<![a-z0-9_-]){re.escape(selector)}(?:\b|::)", runtime_text, re.I):
            errors.append(f"runtime author-facing classes must stay in the * namespace: {selector}")
    if ".highlight" not in runtime_text:
        errors.append("runtime must expose the namespaced .highlight helper")
    if ".copy-center" not in runtime_text:
        errors.append("runtime must expose the split-layout .copy-center helper")
    if ".main-fill" not in runtime_text:
        errors.append("runtime must expose the vertical-fill .main-fill helper")
    for contract in (
        'data-bg="grid-wide"', 'data-bg="soft-spotlight"', 'data-bg="block-field"', 'data-bg="media-owned"',
        '.bleed', '.relationship', '.relationship-node', '.relationship-link',
        'data-decor="dots"', 'data-motif="ring"', '.shape-window', 'data-media-treatment="mono"',
    ):
    ...
    for contract in (
        "55%–70%", "浅色比较或特征卡", "垂直居中",
        'data-decor="dots"', "main-fill", "copy-center", "简短内容标签", "可选元素", "把材料写成可以看到的内容", "工具或函数名改成实际动作", "具体对象、动作、数字、文件或状态",
    ):
        if contract not in components_text:
            errors.append(f"components.md must preserve the design contract: {contract}")
    for contract in (
        "slide add <项目> <页面ID>", "build <项目>", "export-pptx", "audit-skill", "data-text-role", "行间", "遮挡", "PPTX",
    ):
    ...
    if 'class="summary"' in browser_source or "lede" in browser_source:
    ...
    if browser_source:
        ...
        with tempfile.TemporaryDirectory(prefix="pptx-style-") as directory:
            project = Path(directory)
            (project / "slides").mkdir()
            shutil.copytree(RUNTIME, project / "runtime")
    ...
    expected_theme = theme_css({"theme": {"palette": "yellow", "typography": "clean", "shape": "soft"}})
    ...
    if "至少四类构图" not in skill_text:
        errors.append("SKILL.md must document composition variety across longer decks")
    return errors
...
def validate_skill() -> None:
    ...
    if errors:
        raise SystemExit("pptx skill validation failed:\n- " + "\n- ".join(errors))


def main() -> int:
    validate_skill()
    print("pptx skill validation passed")
    return 0
