...
def require_project(value: Path) -> Path:
    ...
    if (
        ...
        raise SystemExit(f"Not an initialized pptx project: {project}")
    return project
    ...
    for value in values:
        ...
        for candidate in candidates:
            if deck_path(candidate).is_file() and state_path(candidate).is_file() and candidate not in seen:
                ...
                seen.add(candidate)
    if not found:
        raise SystemExit("No initialized pptx projects found in the supplied paths.")
    return found
...
def default_deck(title: str) -> dict:
    return {
        "title": title,
        "lang": "zh-CN",
        "theme": {"palette": "yellow", "typography": "clean", "shape": "soft", "direction": "fresh-default"},
...
def init_project(target: Path, title: str | None = None) -> Path:
    ...
    for directory in ("slides", "assets", ".pptx"):
