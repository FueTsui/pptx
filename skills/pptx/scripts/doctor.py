#!/usr/bin/env python3
"""Run a self-contained pptx package and HTML workflow health check."""
...
def run_doctor() -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="pptx-doctor-") as directory:
        project = init_project(Path(directory) / "health-check", "pptx health check")
        pptx_cli.slide_add(project, "cover", "A real HTML slide", "statement", None)
...
<section class="slide active" data-slide-id="readable"><div class="slide-safe"><div data-layout style="position:absolute;inset:0"><h1 style="font-size:64px">Readable</h1><span style="font-size:28px">Audience copy</span></div></div></section>
<section class="slide" data-slide-id="small"><div class="slide-safe"><div data-layout style="position:absolute;inset:0"><h1 style="font-size:64px">Later slide</h1><span style="font-size:20px">This explanation is too small.</span><span data-microcopy="index" style="font-size:18px">01</span><span style="font-size:28px;color:#fff;background:#fff">Invisible HTML</span><svg width="400" height="100"><rect width="400" height="100" fill="#fff"/><text x="200" y="60" text-anchor="middle" style="font-size:28px;fill:#fff">Invisible SVG</text></svg></div></div></section>
