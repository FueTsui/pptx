...
- 插画构图必须服从当前页面的真实版位：先在单页 HTML 中确定横竖比例和显示区域，再生成对应构图；主体占最终版位约 65%–80%，避免素材内部自带大面积空边。

## 画风

使用漫画墨水与半调网点，但为投影观看简化细节：
...
- 概念插画优先直接融入页面背景或使用 runtime 的开放媒体组件；不要默认套浅灰圆角卡片。
- 只有色块本身表达边界、状态或层级时才增加容器。浏览器壳、设备壳与文档框只服务真实证据，不用来包装普通插画。
- 照片和有边到边背景的插画可以使用 `object-fit: cover`；UI、文档、图表等不可裁切素材使用 `contain`。通过页面 CSS 的 `object-position` 保护偏离中心的主体。
- 同一页只允许一个主要外框；素材已经自带必须保留的外框时直接保留，不再添加第二层边界。
