"""Deterministic, offline visual catalogs for pptx's HTML-first surface."""
...
def _write(output: Path, contents: str) -> Path:
    ...
    descriptor, temporary_name = tempfile.mkstemp(prefix=".pptx-catalog-", suffix=".html", dir=target.parent)
...
def _starter_srcdoc(path: Path) -> str:
    ...
    css = source.split("/* PPTX-SLIDE-CSS:START */", 1)[1].split("/* PPTX-SLIDE-CSS:END */", 1)[0]
    section = re.search(r"<!-- PPTX-SLIDE:START -->(.*?)<!-- PPTX-SLIDE:END -->", source, re.S)
    if not section:
        raise SystemExit(f"Starter does not contain an PPTX-SLIDE section: {path}")
    ...
    return f'<!doctype html><html><head><style>{runtime}\n{theme}\n{css}</style></head><body data-mode="preview"><div class="slide-preview-viewport"><div class="slide-preview-shell"><div class="slide-preview-stage">{section.group(1)}</div></div></div></body></html>'


def _component_examples() -> str:
    runtime = (ROOT / "assets" / "runtime" / "deck.css").read_text(encoding="utf-8")
    theme = (ROOT / "assets" / "runtime" / "theme.css").read_text(encoding="utf-8")
    demo = '''<div class="component-demo"><div class="panel" data-tone="accent"><span class="label">signal</span><div class="metric"><strong class="metric-value">72%</strong><span class="metric-label">关键指标</span></div></div><blockquote class="quote">共享 primitives 负责几何与表现。</blockquote></div>'''
    owned_media = '<i class="catalog-media-owned-sample" aria-hidden="true"></i>'
    backgrounds = ''.join(
        f'<div class="catalog-bg"><section class="slide s-catalog" data-bg="{name}"><span>{name}</span>'
    ...
    cards = [
        ("layout", "stack · grid · head · notes · surface", '<div class="component-demo"><div class="grid" style="--grid-columns:3"><div class="panel">A</div><div class="panel">B</div><div class="panel">C</div></div></div>'),
        ("content", "panel · metric · quote · label", demo),
        ...
        ("media treatments", "natural · muted · mono · overlays · masks · bleed", '<div class="component-demo catalog-media"><div class="media" data-media-treatment="natural"><div class="media-placeholder">NATURAL</div></div><div class="media" data-media-treatment="muted" data-overlay="accent-wash"><div class="media-placeholder">MUTED</div></div><div class="media" data-media-treatment="mono" data-mask="fade-bottom"><div class="media-placeholder">MONO</div></div><div class="bleed" data-side="right"><span>BLEED</span></div></div>'),
        ("data", "timeline · chart · table · connector", '<div class="component-demo"><div class="chart"><i class="chart-bar" style="--bar-level:42%"></i><i class="chart-bar" style="--bar-level:72%" data-focus="true"></i><i class="chart-bar" style="--bar-level:56%"></i></div></div>'),
        ("surface craft", "dots · ring · triangle · slash (one craft layer per surface)", '<div class="component-demo catalog-surfaces"><div class="surface" data-decor="dots">DOTS</div><div class="surface" data-motif="ring"><span class="shape-window" aria-hidden="true"></span>RING</div><div class="surface" data-motif="triangle">TRIANGLE</div><div class="surface" data-motif="slash">SLASH</div></div>'),
        ("relationship", "relationship · relationship-node · relationship-link", '<div class="component-demo catalog-relationship"><div class="relationship"><i class="relationship-link" data-direction="arrow"></i><strong class="relationship-node surface" data-tone="accent">CENTER</strong><span class="relationship-node surface">INPUT</span></div></div>'),
    ]
    rendered = []
    for name, label, markup in cards:
        srcdoc = f'<!doctype html><html><head><style>{runtime}\n{theme}\nhtml,body{{overflow:hidden!important;background:#fff}}.component-demo{{position:relative;width:1920px;min-height:1080px;padding:128px;font-size:52px}}.component-demo .panel{{padding:64px}}.component-demo .grid{{gap:36px}}.component-demo .metric-value{{font-size:112px}}.component-demo .metric-label{{font-size:34px}}.component-demo .quote{{margin-top:34px;padding:34px 44px;font-size:46px}}.component-demo .chart{{height:760px;gap:34px;padding:80px 0}}.catalog-backgrounds{{display:grid;grid-template-columns:repeat(4,1fr);gap:24px;padding:72px}}.catalog-bg{{height:420px;overflow:hidden;border-radius:24px}}.catalog-bg .slide{{position:relative!important;display:block!important;visibility:visible!important;opacity:1!important;width:100%!important;height:420px!important;padding:34px;color:var(--ink-2);font:700 28px/1 var(--font-ui)}}.catalog-bg .catalog-media-owned-sample{{position:absolute;right:0;top:0;width:58%;height:100%;background:linear-gradient(135deg,var(--accent-soft),var(--surface));clip-path:polygon(20% 0,100% 0,100% 100%,0 100%)}}.catalog-media{{display:grid;grid-template-columns:repeat(3,1fr);gap:28px;overflow:hidden}}.catalog-media .media{{min-height:650px;border-radius:28px}}.catalog-media .media-placeholder{{font-size:34px}}.catalog-media .bleed{{position:relative;grid-column:1/-1;width:100%;height:104px;display:grid;place-items:center;color:var(--ink-2);font:700 30px/1 var(--font-ui)}}.catalog-surfaces{{display:grid;grid-template-columns:repeat(4,1fr);gap:28px}}.catalog-surfaces .surface{{display:grid;place-items:center;min-height:710px;padding:28px;border-radius:32px;color:var(--ink-2);font:700 34px/1 var(--font-ui)}}.catalog-relationship .relationship{{height:824px}}.catalog-relationship .relationship-link{{left:430px;top:405px;width:1000px;height:5px}}.catalog-relationship .relationship-node{{display:grid;place-items:center;padding:38px;min-width:300px;min-height:150px;border-radius:28px;font:700 34px/1 var(--font-ui)}}.catalog-relationship strong{{left:90px;top:330px}}.catalog-relationship span{{right:90px;top:330px}}</style></head><body data-mode="preview">{markup}</body></html>'
...
def render_starter_catalog(output: Path | None = None) -> Path:
    ...
    body = f'''<p class="eyebrow">PPTX-PPT / HTML-FIRST</p><h1>Starter 与通用组件总览</h1><p>每个 starter 预览都由真实独立 HTML source 的 section 和 page-scoped CSS 生成；无需页面 JSON schema。</p><h2>Composition families</h2><p>正常 8–10 页叙事至少混用四类构图，避免连续重复同一种卡片墙。</p><ul>{family_list}</ul><h2>Starters · {len(starters)}</h2><div class="grid">{"".join(cards)}</div><h2>Generic .* primitives</h2><p>固定示例来自当前 deck.css，可组合进任意独立页面。</p><div class="grid">{_component_examples()}</div>'''
    return _write(output or Path("~/pptx-starter-catalog.html"), _page("pptx starter catalog", body))
