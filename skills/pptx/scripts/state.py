...
def read_state(project: Path) -> dict:
    ...
    if not path.is_file():
        return {"schema_version": "pptx.state/v1"}
    try:
...
def write_state(project: Path, **updates: object) -> dict:
    ...
    value.setdefault("schema_version", "pptx.state/v1")
    write_json(state_path(project), value)
    return value


def input_manifest(project: Path) -> dict[str, str]:
    """Source and generation dependencies whose changes invalidate generated outputs."""
    ...
    for name in ("slide_html.py", "preview_deck.py", "build_deck.py", "project.py", "theme.py", "cdp_validate.py", "pptx_export.py", "html_urls.py", "media_assets.py"):
        path = renderer / name
        if path.is_file():
