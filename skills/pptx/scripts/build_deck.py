...
def chrome_binary() -> str | None:
    ...
    candidates = [os.environ.get("CHROME_BIN", ""), which("google-chrome") or "", which("chromium") or "", which("chrome") or "", which("msedge") or "", "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"]
    for variable in ("PROGRAMFILES", "PROGRAMFILES(X86)", "LOCALAPPDATA"):
        location = os.environ.get(variable)
        if location:
            candidates.extend(str(Path(location) / relative) for relative in ("Google/Chrome/Application/chrome.exe", "Microsoft/Edge/Application/msedge.exe"))
...
def build_project(project_value: Path, *, browser: bool = True) -> Path:
    ...
    title = html.escape(str(deck.get("title") or "pptx"))
    ...
    descriptor, temporary_name = tempfile.mkstemp(prefix=".pptx-build-", suffix=".html", dir=project)
    ...
    try:
        ...
        if temporary.exists():
            temporary.unlink()
    import hashlib
    write_state(project, phase="ready_to_export", build=str(output), build_sha256=hashlib.sha256(output.read_bytes()).hexdigest(), build_manifest=input_manifest(project), browser_issues=None)
    return output
