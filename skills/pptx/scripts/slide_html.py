...
CSS_START = "/* PPTX-SLIDE-CSS:START */"
CSS_END = "/* PPTX-SLIDE-CSS:END */"
HTML_START = "<!-- PPTX-SLIDE:START -->"
HTML_END = "<!-- PPTX-SLIDE:END -->"
...
def _has_overloaded_repeated_lanes(slide: Slide) -> bool:
    ...
    for node in repeatable:
        for token in node.classes - {"panel", "surface"}:
...
def _implementation_identifiers(nodes: list[_CompositionNode]) -> set[str]:
    ...
    for node in nodes:
        if node.tag in {"code", "pre", "script", "style"} or node.classes & {"code"}:
            continue
...
def parse_slide(path: Path, expected_id: str | None = None) -> Slide:
    ...
    css = _single(text, CSS_START, CSS_END, path, "PPTX-SLIDE-CSS")
    fragment = _single(text, HTML_START, HTML_END, path, "PPTX-SLIDE")
    matches = list(SECTION_RE.finditer(fragment))
    if len(matches) != 1 or matches[0].group(0).strip() != fragment.strip():
        _fail(path, "PPTX-SLIDE markers must contain exactly one section")
    attrs = _attrs(matches[0].group("attrs"))
    classes = set(attrs.get("class", "").split())
    slide_id = attrs.get("data-slide-id", "")
    if not ID_RE.fullmatch(slide_id) or "slide" not in classes or f"s-{slide_id}" not in classes:
        _fail(path, "section requires slide, matching s-<id>, and a stable data-slide-id")
...
def _style_advice_for_slide(slide: Slide) -> list[dict[str, str]]:
    ...
    has_browser_showcase = any("browser" in node.classes for node in nodes)
    has_external_summary = any(node.classes & {"summary", "lede"} for node in nodes)
    ...
    panel_count = len(re.findall(r"\bclass\s*=\s*['\"][^'\"]*\bpanel\b", slide.section, re.I))
    ...
    for match in re.finditer(r"<(?:div|article|section)\b([^>]*)>", slide.section, re.I | re.S):
        ...
        is_surface = classes and ("surface" in classes.group(2).split() or "panel" in classes.group(2).split())
...
{CSS_START}
.s-{slide_id} .slide-title {{ margin: 0; font-size: var(--text-title); line-height: var(--leading-display); letter-spacing: var(--tracking-display); }}
.s-{slide_id} .slide-subtitle {{ margin: 28px 0 0; font-size: 30px; line-height: var(--leading-body); color: var(--ink-2); }}
{CSS_END}
...
{HTML_START}
<section class="slide s-{slide_id}" data-slide-id="{slide_id}" data-title="{safe_title}"><div class="slide-safe"><h1 class="slide-title" data-text-role="title">{safe_title}</h1><p class="slide-subtitle" data-text-role="lead">Edit this slide directly in HTML.</p></div></section>
{HTML_END}
