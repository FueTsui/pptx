...
def validate_file(
    ...
    """
    profile_ctx = tempfile.TemporaryDirectory(prefix="pptx-cdp-")
    ...
    try:
        ...
        expression = r"""(() => {
          ...
          const slideId = node => node.closest('.slide')?.dataset.slideId || 'unknown';
          const describe = node => {
          ...
          const overflowTextElements = [...new Set([
            ...all(semanticTextSelector).filter(node => node.closest('.slide')),
            ...textElements,
          ])];
          ...
          const minimumTextSize = node => {
            ...
            const role = node.closest('[data-text-role]')?.getAttribute('data-text-role');
            if (['caption', 'label', 'code'].includes(role)) return 20;
            if (node.closest('pre,code,.code,figcaption,cite,th,small,.label,.tag,.browser-bar,.media-placeholder')) return 20;
            return 24;
          };
