#!/usr/bin/env python3
"""pptx 1.0: author each presentation page as standalone HTML."""
...
from workflow import AUTHORING_RULE, _current_delivery, batch as workflow_batch
...
def cli_command(*parts: object) -> str:
    if os.name == "nt":
        values = [sys.executable, "-X", "utf8", str((SCRIPTS / "pptx_cli.py").resolve()), *(str(part) for part in parts)]
        return "& " + " ".join("'" + value.replace("'", "''") + "'" for value in values)
    return shlex.join([str((SCRIPTS / "pptx").resolve()), *(str(part) for part in parts)])
...
def export_pptx_project(project_value: Path, output_value: Path | None = None, report_value: Path | None = None) -> dict:
    """Generate current HTML/PPTX and bind their editability metadata to the files."""
    ...
    if output == report:
        raise SystemExit("PPTX and editability metadata must have distinct paths.")
    if state.get("preview_manifest") != input_manifest(project) or not (project / "预览.html").is_file():
        render_preview(project)
    # A prior final HTML may belong to an older manifest, so export never trusts it.
    build_project(project, browser=True)
    manifest = input_manifest(project)
    write_state(project, phase="exporting_pptx")
    result = export_hybrid_pptx(
        project=project,
        deck=deck,
    ...
    if not result.get("ok"):
        write_state(project, phase="repair_pptx")
        return {**result, "next": {"action": "run_command", "command": cli_command("build", project)}}
    delivery_state = write_state(
        project,
        phase="complete",
        pptx=str(output),
        pptx_sha256=hashlib.sha256(output.read_bytes()).hexdigest(),
        pptx_manifest=manifest,
        pptx_editability=str(report),
        pptx_editability_sha256=hashlib.sha256(report.read_bytes()).hexdigest(),
    )
    passed = input_manifest(project) == manifest and _current_delivery(project, delivery_state, manifest, slide_count=len(deck["slides"]))
    if not passed:
        write_state(project, phase="repair_pptx")
    return {**result, "ok": passed, "artifact": str(output), "html": str(project / "演示文稿.html"), "editability_report": str(report), "next": {"action": "complete", "command": None} if passed else {"action": "run_command", "command": cli_command("build", project), "brief": "Generated HTML, PPTX, or editability metadata does not match the current inputs. Rebuild the delivery."}}
...
def parser() -> argparse.ArgumentParser:
    ...
    root.add_argument("--version", action="version", version="pptx 1.0.0")
    ...
    init = sub.add_parser("init", help="create an HTML-first pptx project")
    ...
    build = sub.add_parser("build", help="generate current HTML, editable PPTX, and editability metadata")
    build.add_argument("project", type=Path); build.add_argument("--browser", action="store_true", help="browser validation is always enabled"); build.add_argument("--output", type=Path); build.add_argument("--report", type=Path); build.add_argument("--compact", action="store_true")
    export = sub.add_parser("export-pptx", help="rebuild current HTML and export editable PPTX with matching metadata")
    ...
    audit = sub.add_parser("audit-pptx", help="maintenance-only: inspect one PPTX fixture; normal generation does not run this")
    audit.add_argument("pptx", type=Path); audit.add_argument("--report", type=Path); audit.add_argument("--render-dir", type=Path); audit.add_argument("--rules", type=Path); audit.add_argument("--project", type=Path); audit.add_argument("--source-report", type=Path); audit.add_argument("--compact", action="store_true")
    skill_audit = sub.add_parser("audit-skill", help="maintenance-only: generate a skill audit after changing the pptx skill")
    skill_audit.add_argument("--output", type=Path, required=True); skill_audit.add_argument("--chrome"); skill_audit.add_argument("--compact", action="store_true")
...
def main() -> None:
    ...
    if args.command in {"build", "export-pptx"}:
        result = export_pptx_project(args.project, args.output, args.report)
        emit(result, compact)
        if not result.get("ok"):
            raise SystemExit(1)
        return
    if args.command == "audit-pptx":
        ...
        result = audit_pptx(args.pptx, report_path=args.report, render_dir=args.render_dir, rules_path=args.rules, require_render=True, project=args.project, source_report=args.source_report)
        emit(result, compact)
        if not result.get("ok"):
            raise SystemExit(1)
        return
    if args.command == "audit-skill":
        from audit_skill import audit_skill
        result = audit_skill(args.output, chrome=args.chrome)
        emit(result, compact)
        if not result.get("ok"):
            raise SystemExit(1)
        return
